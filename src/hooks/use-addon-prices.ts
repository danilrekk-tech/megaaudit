...
import { getAddonPricesViaApi, usesRemoteApi } from "@/lib/api-client";
...
export function useAddonPrices(addons: Addon[]) {
  ...
  useEffect(() => {
    ...
    const request = usesRemoteApi
      ? getAddonPricesViaApi(requested)
      : loadPrices({ data: { addons: requested } });
